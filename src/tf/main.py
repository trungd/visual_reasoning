...
from dlex import Configs, logger
from dlex_impl.mac.src.clevr import CLEVR
...
def statsToStr(stats, res, epoch, batchNum, dataLen):
    formatStr = "\rl = {loss}, a = {acc}"
    ...
    return formatStr.format(
        epoch=s_epoch, batchNum=s_batchNum, dataProcessed=s_dataProcessed,
        dataLen=s_dataLen, lr=s_lr, loss=s_loss, acc=s_acc)
...
def main():
    ...
    params.gpu = [0]
    # process data
    dataset_builder = CLEVR(params)
    dataset = dataset_builder.get_tensorflow_wrapper("train")
    # preprocessor = CLEVR(configs.environments[0].configs_list[0])
    # data, embeddings, answerDict = preprocessor.preprocessData()
    data = dataset.data

    # build model
    logger.info("Building model...")
    model = MAC(params, dataset)
    train_op = model.get_train_op()
    metric_ops = model.get_metric_ops()
    ...
    with tf.Session(config=sessionConfig) as sess:
        ...
        epoch = loadWeights(sess, None, init)
        ...
        if config.train:
            ...
            for epoch in range(config.restoreEpoch + 1, config.epochs + 1):
                ...
                start = time.time()
                stats = initStats()
                batches = getBatches(data, batchSize=config.batchSize)
                random.shuffle(batches)

                for batchNum, batch in enumerate(batches):
                    feed = {}
                    model.populate_feed_dict(feed, True)
                    dataset.populate_feed_dict(feed, model.placeholders, batch)

                    _, *metrics = sess.run(
                        [train_op, *list(metric_ops.values())],
                        feed_dict=feed)

                    res = {metric: val[1] for metric, val in zip(metric_ops.keys(), metrics)}
                    ...
                    sys.stdout.write(statsToStr(stats, res, epoch, batchNum, len(data)))
                    sys.stdout.flush()
                ...
                trainRes = {"loss": stats["loss"], "acc": stats["acc"]}
